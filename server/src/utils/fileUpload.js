...
export const storeUploadInvoiceImages = async ({ stream, filename, mimetype }) =>
{

    const id = `${new Date().getTime()}${path.extname(filename)}`;

    const _path = path.resolve(BASE_DIR + "/invoiceImages", `${id}`);
...
export const processUploadInvoiceImages = async upload =>
{
    const { createReadStream, filename, mimetype } = await upload;
    const stream = createReadStream();
    const file = await storeUploadInvoiceImages({ stream, filename, mimetype });
    return file;
};